...
class PrintController extends AppController {
  ...
  public function faculty(){
    ...
    $tmpData = $this->Employees->query($this->Employees->getAllEmployeePrint($conditions));
  ...
  public function referralSlip($id = null){
    ...
    $pdf->SetFont("Arial", '', 9);
    $y = $pdf->GetY();
    $pdf->MultiCell(100,5,$data['CollegeProgram']['name'].' - '.properCase($data['ReferralSlip']['year']),0,1);
    $pdf->setXY(3,$y);
 ...
 public function referral_recommendation_form($id = null){
    ...
    $tmpData = $this->Dental->query($this->Dental->getAllDental($conditions));
   ...
   public function dental_form($id = null){

    $office_reference = $this->Global->OfficeReference('Dental');

    $data = $this->Dental->find('first', array(
    ...
    $pdf->Cell(47,5,'',0,0,'L');
    $pdf->Cell(15,5,'Date: ',0,0,'L');
    $pdf->Cell(65,5,fdate($data['Dental']['date'],'m/d/Y'),0,0,'L');
    $pdf->Line(190,$pdf->getY()+5,135,$pdf->getY()+5);
    $pdf->Ln(6);
    $pdf->SetFont("Arial", '', 9.5);
    $pdf->Cell(2,5,'',0,0,'L');
    $pdf->Cell(29,5,'COURSE AND YEAR: ',0,0,'L');
    $pdf->Cell(47,5,strtoupper($data['Dental']['year']." - ".$data['Course']['code']),0,0,'R');
  ...
  public function property_log(){
    ...
    $tmpData = $this->PropertyLog->query($this->PropertyLog->getAllPropertyLog($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){
        ...
        $tmp = $data['PropertyLog'];

        $pdf->RowLegalP(array(

          $key + 1,

          strtoupper($tmp['property_name']),

          fdate($data['PropertyLog']['date'],'m/d/Y'),

          fdate($tmp['manufacturing_date'],'m/d/Y'),

          strtoupper($tmp['batch_no']),

          fdate($tmp['expiration_date'],'m/d/Y'),
  ...
  public function transferee(){
    ...
    $tmpData = $this->Transferee->query($this->Transferee->getAllTransferee($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){
        ...
        $pdf->RowLegalL(array(
          ...
          $data['Transferee']['student_no'],

          strtoupper($data[0]['full_name']),

          $data['Transferee']['year_level'],  

          $data['Transferee']['email'],

          fdate($data['Transferee']['date'],'m/d/Y'),
 ...
 public function illnessRecommendations(){
  ...
  public function prescription(){
    ...
    $tmpData = $this->Prescription->query($this->Prescription->getAllPrescription($conditions));
  ...
  public function requestForm() {
    ...
    $conditions['status'] = '';
    // var_dump($this->request->getQuery('status'));
    if ($this->request->getQuery('status')==1 ||  $this->request->getQuery('status')==0) {
  ...
  public function registered_students(){
    ...
    $tmpData = $this->RegisteredStudent->query($this->RegisteredStudent->getAllRegisteredStudent($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){
        ...
        $pdf->RowLegalP(array(
          ...
          $data['YearLevelTerm']['description'],

          $data['College']['college'],

          $data['CollegeProgram']['program'],

          $data['Student']['contact_no'],
  ...
  public function report_rating_form($id = null){

    $tmpData = $this->Employee->find('first', array(
      ...
      'conditions' => array(
        ...
        'Employee.id'      => $this->request->query['faculty_id'],
    ...
    $program = $this->CollegeProgram->findById($program_id);

    $course = $this->Course->findById($course_id);

    $year_term = $this->YearLevelTerm->findById($year_term_id);
    ...
    $block_section = $this->BlockSection->find('first', array(
      ...
      'conditions' => array(
        ...
        'BlockSection.section_id' => $section_id,
    ...
    if(!empty($block_section)){
      ...
      $school_year = $block_section['BlockSection']['school_year_start'].' - '.$block_section['BlockSection']['school_year_end'];
    ...
    if(!empty($student)){
      ...
      foreach ($student as $key => $value) {
        ...
        $datas[] = array(
          ...
          'id'              => $value['StudentEnrolledCourse']['id'],

          'code'            => $value['StudentEnrolledCourse']['course'].' :: '.$value['StudentEnrolledCourse']['section'],

          'student_id'      => $value['Student']['id'],

          'student_no'      => $value['Student']['student_no'],

          'student_name'    => $value[0]['full_name'],

          'midterm_grade'   => $value['StudentEnrolledCourse']['midterm_grade'],

          'finalterm_grade' => $value['StudentEnrolledCourse']['finalterm_grade'],

          'final_grade'     => $value['StudentEnrolledCourse']['final_grade'],

          'remarks'         => $value['StudentEnrolledCourse']['remarks'],
    ...
    $pdf->SetFont("Times", 'B', 15);
    $pdf->Cell(0, 5, $tmpData['College']['name'], 0, 0, 'C');
    $pdf->Ln(6);
    $pdf->SetFont("Times", 'B', 15);
    ...
    $pdf->Cell(70, 5, @$program['CollegeProgram']['code'], 0, 0, 'L');

    $pdf->Cell(19, 5, "Semester:", 0, 0, 'L');
    $check = "4";
    $pdf->Cell(2, 5, '', 0, 0, 'L');
    $pdf->Cell(12, 5, "First ", 0, 0, 'L');
    if ($year_term['YearLevelTerm']['semester'] == '1st Semester'){
    ...
    if ($year_term['YearLevelTerm']['semester'] == '2nd Semester'){
      $pdf->Cell(1.7, 4, '(    )', 0, 0, 'L');
      $pdf->SetFont('ZapfDingbats', '', 10);
      $pdf->Cell(4, 4, $check, 0, 0, 'L');
      $pdf->SetFont("Times", '', 10);
    ...
    if ($year_term['YearLevelTerm']['semester'] == 'Summer'){
      $pdf->Cell(1.7, 4, '(    )', 0, 0, 'L');
      $pdf->SetFont('ZapfDingbats', '', 10);
      $pdf->Cell(4, 4, $check, 0, 0, 'L');
      $pdf->SetFont("Times", '', 10);
    ...
    $pdf->Cell(50, 5, @$program['CollegeProgram']['name'], 0, 0, 'L');
    $pdf->Line(35, $pdf->getY() + 5, 85, $pdf->getY() + 5);
    $pdf->Cell(12, 5, "Course:", 0, 0, 'L');
    $pdf->Cell(50, 5, @$course['Course']['title'], 0, 0, 'L');
    $pdf->Line(96, $pdf->getY() + 5, 208, $pdf->getY() + 5);
    $pdf->Ln(6);
    $pdf->Cell(2, 5, '', 0, 0, 'L');
    $pdf->Cell(73, 5, "", 0, 0, 'L');
    // $pdf->Line(25, $pdf->getY() + 5, 95, $pdf->getY() + 5);
    $pdf->Cell(26, 5, "Year/Section:", 0, 0, 'L');
    $pdf->Cell(55, 5,@$section['Section']['name'], 0, 0, 'L');
    ...
    if(!empty($datas)){
      ...
      foreach ($datas as $key => $data){
        ...
        $pdf->RowLegalP(array(
          ...
          $data['midterm_grade'] > 0 ? fnumber($data['midterm_grade'],2) : '',

          $data['finalterm_grade'] > 0 ? fnumber($data['finalterm_grade'],2) : '',
  ...
  public function apartelle_registration(){
    ...
    $tmpData = $this->ApartelleRegistration->query($this->ApartelleRegistration->getAllApartelleRegistration($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){ 
        ...
        $tmp = $data['ApartelleRegistration'];
        ...
        $pdf->RowLegalL(array(
          ...
          $data['ApartelleRegistration']['code'],

          $data['ApartelleRegistration']['student_name'],

          $data['ApartelleRegistration']['nick_name'],

          fdate($data['ApartelleRegistration']['date_of_birth'],'m/d/Y'),  

          $data['ApartelleRegistration']['address'],

          $data['CollegeProgram']['code'],
  ...
  public function list_apartelle(){
    ...
    $tmpData = $this->Apartelle->query($this->Apartelle->getAllApartelle($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){ 
        ...
        $pdf->RowLegalL(array(
          ...
          $data['Apartelle']['building_no'],

          $data['Apartelle']['room_no'],

          $data['Apartelle']['price'],

          $data['Apartelle']['capacity']
...
// print_r($data);
    ...
    $pdf->Cell(1, 5, '', 0, 0, 'L');
    $pdf->MultiCell(105, 3, '
    Please log on to your account to verify the subjects enrolled and other information at
    ...
    $today = date("F d, Y");
    $pdf->SetFont("Arial", '', 10);
    $pdf->SetXY(116, $pdf->getY()-24);
    $pdf->MultiCell(50, 3, '
    
    Printed By:'.$data['Student']['student_no'].'
    
    Date Printed: '.$today .'


    ', 1, 1);
    $pdf->Image($this->base . '/assets/img/zscmst-qr.png', 50, $pdf->getY()-17, 15, 15);
 ...
 public function inventoryBibliography(){
  ...
  public function item_issuances() {
    ...
    $tmpData = $this->ItemIssuance->query($this->ItemIssuance->getAllItemIssuance($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){
        ...
        $tmp = $data['ItemIssuance'];

        $pdf->RowLegalL(array(

          $key + 1,

          $data['ItemIssuance']['code'],

          strtoupper($data['ItemIssuance']['type']),

          fdate($data['ItemIssuance']['date'],'m/d/Y'),  
...
// print_r($data);
  ...
  public function cat_application_form($id = null){
    ...
    $pdf->Ln(6);
    $pdf->Cell(11);
    $pdf->Cell(10,4,fdate($data['StudentApplication']['date'],'m/d/Y'),0,0,'L');
    $pdf->Cell(47);
    ...
    $pdf->Ln(6);
    $pdf->Cell(11);
    $pdf->Cell(10,4,fdate($data['StudentApplication']['date'],'m/d/Y'),0,0,'C');
    $pdf->Cell(25);
  ...
  public function add_drop(){
    ...
    $tmpData = $this->AddingDroppingSubject->query($this->AddingDroppingSubject->getAllAddingDroppingSubject($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){
        ...
        $tmp = $data['AddingDroppingSubject'];

        $status = '';

        if($data['AddingDroppingSubject']['approve'] == 0){

          $status = 'PENDING';

        }elseif($data['AddingDroppingSubject']['approve'] == 1){

          $status = 'APPROVED';

        }elseif($data['AddingDroppingSubject']['approve'] == 2){
  ...
  public function participant_evaluation_activity() {
    ...
    $tmpData = $this->ParticipantEvaluationActivity->query($this->ParticipantEvaluationActivity->getAllParticipantEvaluationActivity($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){
        ...
        $pdf->RowLegalL(array(
          ...
          $data['ParticipantEvaluationActivity']['activity'],

          $data['ParticipantEvaluationActivity']['venue'],

          $data['CollegeProgram']['name'],

          fdate($data['ParticipantEvaluationActivity']['date'],'m/d/Y'),

          $data['ParticipantEvaluationActivity']['year'],
  ...
  public function program_adviser() {
    ...
    $tmpData = $this->ProgramAdviser->query($this->ProgramAdviser->getAllData($conditions));
    ...
    $pdf->Ln();
    $pdf->SetFont("Arial", '', 8);
    $pdf->SetWidths(array(10,25,70,90));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){ 
        ...
        $pdf->RowLegalL(array(
          ...
          strtoupper($data['CollegeProgram']['name'])
  ...
  public function payment() {
    ...
    $tmpData = $this->Payment->query($this->Payment->getAllPayment($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){ 
        ...
        $pdf->RowLegalL(array(
          ...
          $data['Payment']['student_name'],

          $data['Payment']['program'],

          $data['Payment']['email'],

          $data['Payment']['contact_no'],
  ...
  public function student_ranking(){
    ...
    $tmpData = $this->Report->query($this->Report->getAllStudentRanking($conditions));
  ...
  public function student_club(){
    ...
    $tmpData = $this->StudentClub->query($this->StudentClub->getAllStudentClub($conditions));
  ...
  public function club(){
    ...
    $tmpData = $this->Club->query($this->Club->getAllClub($conditions));
  ...
  public function list_academic_awardees(){
    ...
    if (isset($this->request->query['award'])) {

      $award = $this->request->query['award'];
    ...
    if (!empty($tmpData)) {
      ...
      foreach ($tmpData as $key => $data) {
        ...
        // if ($data['Report']['award'] == 1) {

        //   $award = "President's Lister";

        // } elseif ($data['Report']['award'] == 2) {
  ...
  public function academic_failures_list(){
    ...
    if (isset($this->request->query['club_id'])) {

      $club_id = $this->request->query['club_id']; 
    ...
    $tmpData = $this->Report->query($this->Report->getAllStudentClubList($conditions));
  ...
  public function medical_student_profile(){
    ...
    $tmpData = $this->MedicalStudentProfile->query($this->MedicalStudentProfile->getAllMedicalStudentProfile($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){
        ...
        $pdf->RowLegalL(array(
          ...
          $data['MedicalStudentProfile']['code'],

          $data['MedicalStudentProfile']['student_name'],

          $data['CollegeProgram']['name'],

          $data['YearLevelTerm']['description'],

          $data['MedicalStudentProfile']['address']
  ...
  public function medical_student_profile_form($id = null) {

    $office_reference = $this->Global->OfficeReference('Medical Student Profile');

    $data = $this->MedicalStudentProfile->find('first', array(
      ...
      'conditions' => array(
        ...
        'MedicalStudentProfile.visible' => true,

        'MedicalStudentProfile.id' => $id,
  ...
  public function medical_employee_profile_form($id = null) {
    ...
    $pdf->Cell(8);
    $pdf->Cell(85,5,$data['College']['code']. ' - ' .$data['College']['name'],0,0,'L');
    $pdf->Line(127,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(5);
    $pdf->SetFont("Arial", '', 8.5);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['address'],0,0,'L');
    $pdf->Line(31,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'CIVIL STATUS: ',0,0,'L');
    $pdf->Cell(13);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['civil_status'],0,0,'L');
    $pdf->Line(129,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'WEIGHT: ',0,0,'L');
    $pdf->Cell(5);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['weight'],0,0,'L');
    $pdf->Line(127,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'HEIGHT: ',0,0,'L');
    $pdf->Cell(6);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['height'],0,0,'L');
    $pdf->Line(27,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'RESPIRATORY SYSTEM: ',0,0,'L');
    $pdf->Cell(30);
    $pdf->Cell(100,5,$data['MedicalEmployeeProfile']['respiratory_system'],0,0,'L');
    $pdf->Line(53,$pdf->getY()+5,100,$pdf->getY()+5);
    $pdf->Ln(5);
    $pdf->Line(10,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'EYES: ',0,0,'L');
    $pdf->Cell(5);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['eyes'],0,0,'L');
    $pdf->Line(127,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'COLOR PRECEPTION: ',0,0,'L');
    $pdf->Cell(25);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['color_perception'],0,0,'L');
    $pdf->Line(143,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'FLOUROGRAPHY: ',0,0,'L');
    $pdf->Cell(20);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['flourography'],0,0,'L');
    $pdf->Line(42,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'VISION: ',0,0,'L');
    $pdf->Cell(5);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['vision'],0,0,'L');
    $pdf->Line(129,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'LUNGS: ',0,0,'L');
    $pdf->Cell(5);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['lungs'],0,0,'L');
    $pdf->Line(26,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'FAR RIGHT: ',0,0,'L');
    $pdf->Cell(10);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['far_right'],0,0,'L');
    $pdf->Line(129,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'HEART: ',0,0,'L');
    $pdf->Cell(5);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['heart'],0,0,'L');
    $pdf->Line(26,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'LEFT: ',0,0,'L');
    $pdf->Cell(1);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['far_left'],0,0,'L');
    $pdf->Line(129,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'SYSTOLIC: ',0,0,'L');
    $pdf->Cell(10);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['systolic'],0,0,'L');
    $pdf->Line(31,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'EARS: ',0,0,'L');
    $pdf->Cell(-1);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['ears'],0,0,'L');
    $pdf->Line(130,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'DIASTOLIC: ',0,0,'L');
    $pdf->Cell(10);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['diastolic'],0,0,'L');
    $pdf->Line(32,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'HEARING: ',0,0,'L');
    $pdf->Cell(5);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['hearing'],0,0,'L');
    $pdf->Line(130,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'SITTING: ',0,0,'L');
    $pdf->Cell(10);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['sitting'],0,0,'L');
    $pdf->Line(28,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'THROAT: ',0,0,'L');
    $pdf->Cell(3.5);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['throat'],0,0,'L');
    $pdf->Line(130,$pdf->getY()+5,200,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'AGILITY TESTS: ',0,0,'L');
    $pdf->Cell(20);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['agility_test'],0,0,'L');
    $pdf->Line(42,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'IMMUNIZATION: ',0,0,'L');
    $pdf->Cell(15);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['immunization'],0,0,'L');
    $pdf->Line(130,$pdf->getY()+5,200,$pdf->getY()+5);
    $pdf->Ln(6);
    $pdf->Line(120,$pdf->getY()+5,200,$pdf->getY()+5);
    $pdf->Ln(6);
    ...
    $pdf->Cell(15,5,'DIGESTIVE SYSTEM: ',0,0,'L');
    $pdf->Cell(20);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['digestive_system'],0,0,'L');
    $pdf->Line(47,$pdf->getY()+5,100,$pdf->getY()+5);
    $pdf->Ln(6);
    $pdf->Line(15,$pdf->getY()+5,100,$pdf->getY()+5);
    $pdf->Ln(6);
    ...
    $pdf->Cell(15,5,'REMARKS: ',0,0,'L');
    $pdf->Cell(15);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['remarks'],0,0,'L');
    $pdf->Line(122,$pdf->getY()+5,200,$pdf->getY()+5);
    $pdf->Ln(6);
    $pdf->Line(110,$pdf->getY()+5,200,$pdf->getY()+5);
    $pdf->Ln(6);
    ...
    $pdf->Cell(15,5,'RECOMMENDATIONS: ',0,0,'L');
    $pdf->Cell(25);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['recommendation'],0,0,'L');
    $pdf->Line(140,$pdf->getY()+5,200,$pdf->getY()+5);
    $pdf->Ln(6);
    $pdf->Line(110,$pdf->getY()+5,200,$pdf->getY()+5);
    $pdf->Ln(6);
    ...
    $pdf->Cell(15,5,'URINALYSIS: ',0,0,'L');
    $pdf->Cell(15);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['urinalysis'],0,0,'L');
    $pdf->Line(34,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'SKIN: ',0,0,'L');
    $pdf->Cell(15);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['skin'],0,0,'L');
    $pdf->Line(22,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'LOCOMOTION SYSTEM: ',0,0,'L');
    $pdf->Cell(35);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['locomotion_system'],0,0,'L');
    $pdf->Line(52,$pdf->getY()+5,100,$pdf->getY()+5);
    ...
    $pdf->Cell(15,5,'NERVOUS SYSTEM: ',0,0,'L');
    $pdf->Cell(25);
    $pdf->Cell(85,5,$data['MedicalEmployeeProfile']['nervous_system'],0,0,'L');
    $pdf->Line(46,$pdf->getY()+5,100,$pdf->getY()+5);
  ...
  public function medical_employee_profile(){
    ...
    $tmpData = $this->MedicalEmployeeProfile->query($this->MedicalEmployeeProfile->getAllMedicalEmployeeProfile($conditions));
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $key => $data){
        ...
        $pdf->RowLegalL(array(
          ...
          $data['MedicalEmployeeProfile']['code'],

          $data['MedicalEmployeeProfile']['employee_name'],

          $data['MedicalEmployeeProfile']['address'],

          $data['MedicalEmployeeProfile']['age'],

          $data['MedicalEmployeeProfile']['height'],

          $data['MedicalEmployeeProfile']['weight'],

          $data['MedicalEmployeeProfile']['remarks']
  ...
  public function medical_history_form($id = null) {
  ...
  public function export_registered_students(){
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $data) {
        ...
        $datas[] = array(
          ...
          'id'          => $data['StudentEnrollment']['student_id'],

          'full_name'   => $data[0]['full_name'],

          'student_no'  => $data['StudentEnrollment']['student_no'],

          'college'     => $data['College']['college'],

          'program'     => $data['CollegeProgram']['program'],

          'email'       => $data['Student']['email'],

          'contact_no'  => $data['Student']['contact_no'],

          'year'        => $data['YearLevelTerm']['description']
  ...
  public function export_bibliography() {
    ...
    if(!empty($tmpData)){
      ...
      foreach ($tmpData as $data) {
        ...
        $datas[] = array(
          ...
          'material_type' => $data['MaterialType']['material_type'], 

          'collection_type' => $data['CollectionType']['collection_type'],

          'date_of_publication' => fdate($data['Bibliography']['date_of_publication'],'m/d/Y'),
