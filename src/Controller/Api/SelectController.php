...
class SelectController extends AppController {
  ...
  public function beforeFilter(\Cake\Event\EventInterface $event){
    ...
    if($code == 'all-permissions'){
      ...
      $tmp = $this->MedicalStudentProfile->query("
        ...
        FROM 
          ...
          medical_student_profiles as MedicalStudentProfile

      ");
   
      $datas = 'MSP-' . str_pad($tmp[0][0]['total'] + 1, 5, "0", STR_PAD_LEFT);
      ...
      $application_id = $this->request->query['id'];

      $student = $this->StudentApplication->findById($application_id);

      $program_list = $this->CollegeProgram->find('all', array(

        'conditions' => array(

          'CollegeProgram.visible' => true,

          'CollegeProgram.passing_rate <=' => $student['StudentApplication']['rate']
      ...
      if(!empty($program_list)){
        ...
        foreach ($program_list as $k => $data) {
          ...
          $datas[] = array(
            ...
            'value'   => $data['CollegeProgram']['name']
      ...
      $datas = $this->StudentApplication->findById($application_id);
      ...
      $tmp = $this->StudentEnrollment->query("
        ...
        FROM
          ...
          courses as Course ON Course.id = Student.course_id  
      ...
      if(!empty($tmp)){
            ...
            foreach ($tmp as $key => $value) {
              ...
              $datas[] = array(
                ...
                'StudentEnrollment'   => $value['StudentEnrollment']['id'],
      ...
      if(!empty($tmp)){
        ...
        foreach ($tmp as $k => $data) {
          ...
          $datas[] = array(
            ...
            'value'   => $data['Club']['code'].' - '.$data['Club']['title']
      ...
      $tmp = $this->StudentClub->query("
        ...
        FROM 
          ...
          student_clubs as StudentClub

      ");
   
      $datas = 'STC-' . str_pad($tmp[0][0]['total'] + 1, 5, "0", STR_PAD_LEFT);
     ...
     $tmp = $this->AttendanceCounselings->find()->where([
      ...
      $tmp = $this->ApartelleRegistration->query("
      ...
      $datas = 'ADR-' . str_pad($tmp[0][0]['total'] + 1, 5, "0", STR_PAD_LEFT);
      ...
      $tmp = $this->AddingDroppingSubject->query("
        ...
        FROM 
          ...
          adding_dropping_subjects as AddingDroppingSubject

      ");
   
      $datas = 'ADS-' . str_pad($tmp[0][0]['total'] + 1, 5, "0", STR_PAD_LEFT);
      ...
      $tmp = $this->Apartelle->find('all', array(
      ...
      if(!empty($tmp)){
        ...
        foreach ($tmp as $k => $data) {
          ...
          $id =$data['Apartelle']['id'];
          
          $query = $this->ApartelleRegistration->find('count', array(

            'conditions' => array(

              'ApartelleRegistration.visible' => true,

              'ApartelleRegistration.apartelle_id' => $id
          ...
          if(is_numeric($data['Apartelle']['capacity'])){

            $counter = $data['Apartelle']['capacity'] - $query;
          ...
          if($counter>0){
            ...
            'value' => $data['Apartelle']['building_no']. ' - ' .$data['Apartelle']['room_no']. ' - Capacity : ' .$counter,
      ...
      if(!empty($student)){
        ...
        foreach ($student as $key => $value) {
          ...
          $datas[] = array(
            ...
            'year_term_id'        => $value['Student']['year_term_id'],
            ...
            'midterm_submitted'   => $value['StudentEnrolledCourse']['midterm_submitted'],

            'finalterm_submitted' => $value['StudentEnrolledCourse']['finalterm_submitted'],

            'incomplete'          => $value['StudentEnrolledCourse']['incomplete'],
      ...
      if(isset($this->request->query['school_year_start']) && isset($this->request->query['school_year_end'])){

        $school_year_start = strval($this->request->query['school_year_start']);

        $school_year_end = strval($this->request->query['school_year_end']);
      ...
      if(!empty($tmp)){
        ...
        foreach ($tmp as $k => $data) {
          ...
          $datas[] = array(
            ...
            'code'       => $data['ClassSchedule']['code'],

            'day'        => $data['ClassScheduleDay']['day'],

            'time_start' => fdate($data['ClassScheduleDay']['time_start'],'h:i A'),

            'time_end'   => fdate($data['ClassScheduleDay']['time_end'],'h:i A'),

            'room_id'    => $data['ClassScheduleDay']['room_id'],

            'room'       => $data['ClassScheduleDay']['room'],

            'section_id'    => $data['ClassScheduleDay']['section_id'],

            'section'    => $data['ClassScheduleDay']['section'],

            'schedule_id'    => $data['ClassSchedule']['id'],

            'tmp_id'    => $data['ClassScheduleTmp']['id'],
