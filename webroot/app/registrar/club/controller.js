app.controller("ClubController", function ($scope, Club) {
  ...
  $scope.remove = function (data) {
    bootbox.confirm(
      ...
      function (c) {
        if (c) {
          Club.remove({ id: data.id }, function (e) {
            if (e.ok) {
              $.gritter.add({
                title: "Successful!",
...
app.controller(
  ...
  function ($scope, Club, Select) {
    ...
    $scope.save = function () {
      ...
      if (valid) {
        Club.save($scope.data, function (e) {
          if (e.ok) {
            $.gritter.add({
              title: "Successful!",
            ...
            window.location = "#/registrar/club";
          } else {
            $.gritter.add({
              title: "Warning!",
...
app.controller(
  ...
  function ($scope, $routeParams, Club) {
    ...
    $scope.load = function () {
      Club.get({ id: $scope.id }, function (e) {
        $scope.data = e.data;
      });
    };
...
app.controller(
  "ClubEditController",
  function ($scope, $routeParams, Club, Select) {
    $scope.id = $routeParams.id;
